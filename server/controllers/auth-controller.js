  ...
  try {
    ...
    if (userDb) {
      return res.status(422).json({
        error: {
          status: 422,
          data: "Пользователь с таким адресом электронной почты уже существует.",
        },
      });
    }
  ...
  passport.authenticate("login", (err, user, info) => {
    ...
    req.login(user, (err) => {
      ...
      return (
        ...
        res.json({ token, id, first_name, last_name, email, isadmin })
      );
    });
  })(req, res, next);
};
...
const createDocumentByUserController = async (req, res, next) => {
  const { user_id, comment, ip, office } = req.body;
  try {
    const document = {
      user_id,
      comment,
...
const forgotPassLinkController = async (req, res, next) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { email: newDocument.email, id: newDocument.id },
      secret,
      { expiresIn: "5m" }
    );
